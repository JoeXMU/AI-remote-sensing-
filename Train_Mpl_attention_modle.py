# @File : Train_Mpl_attention_modle.py
# @Time : 2024/4/27 9:31
# @Author : Qiao Hanyang
...
if __name__ == '__main__':
    ...
    filtered_box.iloc[:, :17] = robust_scaler.fit_transform(filtered_box.iloc[:, :17])

    # 保存 RobustScaler 对象
    joblib.dump(robust_scaler, 'xxx.pkl')
    ...
    def get_loader(df_total, batch_size, *, rate=0.65):
    ...
    """定义超参数"""
    in_channels, hidden_channels1, hidden_channels2, hidden_channels3, out_channels \
        = 17, 64, 32, 16, 1
    lr = 0.0001
    L2 = 0.1  # L2正则化防止过拟合
    # attention_size = 20
    net = Net(in_channels=in_channels, hid_channels1=hidden_channels1, hid_channels2=hidden_channels2,
              hid_channels3=hidden_channels3, out_channels=out_channels).to(device)
    net = net.to(device)
    ...
    sample = list(range(0, len(onid2), 120))
