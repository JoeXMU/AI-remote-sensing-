# @File : CombinedResnet_remove_lonlat.py
# @Time : 2024/5/29 21:31
# @Author : Qiao Hanyang
...
if __name__ == '__main__':
    ...
    # 保存 RobustScaler 对象
    joblib.dump(robust_scaler, 'Remove lonlat.pkl')
    ...
    class Net(nn.Module):
        def __init__(self, in_channels, out_channels):
            ...
            self.initial_channel_attention = ChannelAttention(25)  # 为原始特征添加通道注意
            self.attention_weights = spatial_attention

            self.layer1 = PyramidResidualBlock(in_channels, 180)
        ...
        def forward(self, x):
            ...
            out = self.layer1(combinedchannel_data)
            out = self.layer2(out)
            out = self.layer3(out)
    ...
    def get_loader(df_total, batch_size, *, rate=0.78):
    ...
    torch.save(net, "Remove lonlat.pth")
    """可视化"""
    ...
    sample = list(range(0, len(onid2), 600))
